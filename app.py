...
def carregar_dados():
  try:
    # Leitura segura para arquivos .xlsb no ambiente do Streamlit Cloud
    with open(CAMINHO_ARQUIVO, "rb") as f:
      file_bytes = f.read()
    df = pd.read_excel(
        io.BytesIO(file_bytes), sheet_name="Apontamentos", engine="pyxlsb"
    )
    ...
    st.error(
        f"Erro interno ao carregar o arquivo binário: {e}. Verifique se o"
        " pacote 'pyxlsb' está no requirements.txt."
    )
    return pd.DataFrame()
...
if not df.empty:
  ...
  if grupo_selecionados and col_grupo:
    df_filtrado = df_filtrado[df_filtrado[col_grupo].isin(grupo_selecionados)]
  ...
  if filtro_atividade and col_atividade:
    df_filtrado = df_filtrado[df_filtrado[col_atividade].isin(filtro_atividade)]
